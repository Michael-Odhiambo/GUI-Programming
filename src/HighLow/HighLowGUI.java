...
public class HighLowGUI extends Application {
    ...
    Image cardImages;   // A single image contains the images of every card.
    ...
    /**
     * The start() method sets up the GUI and event handling. The root pane is a BorderPane.
     * A canvas where cards are displayed occupies the center position of the BorderPane.
     * On the bottom is a HBox that holds three buttons. ActionEvent handlers are set up to call
     * methods defined elsewhere in this class when the user clicks a button.
     * @param stage
     */

    public void start( Stage stage ) {

        cardImages = new Image("HighLow/cards.png" );
        ...
        /**
         * Improve the layout of the buttonBar. Without adjustment, the
         * buttons will be grouped at the left end of the HBox. My solution
         * is to set their preferred widths to make them all the same size
         * and make them fill the entire HBox.
         */
    ...
    /**
     * Called by an event handler when the user clicks the "Higher" button. Check
     * the user's prediction. Game ends if the user guessed wrong or if
     * the user has made three correct predictions.
     */
    ...
    /**
     * Called by an event handler when the user clicks the "Lower" button. Check
     * the user's prediction. Game ends if the user guessed wrong or if
     * the user has made three correct predictions.
     */
    ...
    /**
     * Called by the start() method, and called by an event handler if the
     * user clicks the "New Game" button. Start a new game.
     */
    private void doNewGame() {
        if ( gameInProgress ) {
            // If the current game is not over, it is an error to try to
            // start a new game.
    ...
    /**
     * This method draws the message at the bottom of the canvas, and it
     * draws all of the dealt cards spread out across the canvas. If the game is
     * in progress, an extra card is drawn face down representing the card to
     * be dealt next.
     */
    ...
    private void drawCard( GraphicsContext g, Card card, int x, int y ) {
        int cardRow, cardCol;
        if ( card == null ) {
            cardRow = 4;  // Row and column of face down card.
            cardCol = 2;
