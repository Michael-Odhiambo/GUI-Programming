...
public class MosaicDraw extends Application {
    ...
    private final static int DRAW_3BY3_TOOL = 2;
    private final static int ERASE_3BY3_TOOL = 3;

    private int currentTool;  // The current tool; this is changed when the user makes
                              // a selection from the Tools menu.

    /**
     * The current color. These change when the user selects from the color menu. They are
     * used whenever a square is painted. ( NOTE: I am using three integers to represent the
     * color, rather than a variable of type Color, to make it easier to add in the random color
     * variation. )
     */
    private int currentRed, currentGreen, currentBlue;

    private MosaicCanvas mosaic;  // The panel where the drawing takes place.
    private CheckMenuItem useRandomness;   // If selected, then a small random variation is added to the c
                                           // current color whenever a square is painted. This menu item is
                                           // added to the Control menu in createMenuBar().

    /**
     * If selected, then whenever a square is painted or erased, the three symmetrical squares
     * obtained by reflecting the square vertically and horizontally are also painted or
     * erased. This menu item is added to the Control menu in createMenuBar()
     */
    ...
    public void start( Stage stage ) {
        ...
        // Put the window at screen coords ( 150, 100 ).
        stage.setX( 150 );
        stage.setY( 100 );
        stage.show();
    ...
    /**
     * Creates and returns a menu bar that contains options that affect the drawing
     * that is done on the MosaicCanvas.
     */
    private MenuBar createMenuBar() {
        ...
        controlMenu.getItems().addAll( fill, clear );
        ...
        useGrouting.setOnAction( e ->
                doUseGrouting( useGrouting.isSelected() ) );
        ...
        addRadioMenuItem( toolMenu, "Draw 3*3", toolGroup, false );
        addRadioMenuItem( toolMenu, "Erase 3*3", toolGroup, false );
    ...
    /**
     * Utility method to create a radio menu item, add it to a ToggleGroup, and
     * add it to a menu.
     */
    ...
    /**
     * Fill the mosaic with squares of the current drawing color. ( If symmetry is on, each square actually has
     * its color set four times, so efficiency could be improved. )
     */
    private void doFill() {
        mosaic.setAutopaint(false);  // Turning off autopaint lets all the squares be drawn
        // without inserting any delays; when autopaint is on, there is a one
        // millisecond delay for each square.
    ...
    /**
     * Apply the current tool to the square that contains the mouse pointer. This method is
     * called in response to both MousePressed and MouseDragged events. The mouse handlers that
     * called it are added to the mosaic in the start() method.
     */
    private void doMouse( MouseEvent evt ) {
        ...
        if ( row >= 0 && row < mosaic.getRowCount() && col >= 0 && col < mosaic.getColumnCount() ) {
            // The test in this if statement will be false if the user drags the mouse outside the canvas
            // after pressing the mouse on the canvas. )
    ...
    /**
     * This method is called when the user clicks the "Use Grouting" command in the Control
     * menu. The parameter tells whether the CheckMenuItem is checked. The handler that
     * calls this method is added to the menu item in the createMenuBar() method.
     */
    ...
    /**
     * Changes the current drawing color when the user clicks one of the colors
     * in the Color menu. A handler to call this method is added to each color menu
     * item in the createMenuBar() method. ( Note that this method will be called
     * even if the user picks the color that is already selected, but that is harmless. )
     */
    ...
    /**
     * This method is called when the selected item in the Tools menu is changed for any reason.
     * The handler is added to the ToggleGroup that controls the menu items. Apparently, it is
     * called twice when the user changes the selection, once when the current item is
     * deselected ( with parameter value null ) and once when the new item is selected. The current
     * tool is only changed when the parameter is non-null.
     */
    private void doToolChoice( Toggle toggle ) {
        ...
        else if ( toolName.equals( "Draw 3*3" ) )
            currentTool = DRAW_3BY3_TOOL;
        else if ( toolName.equals( "Erase 3*3" ) )
            currentTool = ERASE_3BY3_TOOL;
    }
}
