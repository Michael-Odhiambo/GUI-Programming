...
/**
 * This program demonstrates how to do your own layout. The root container for the
 * screen is a Pane, which will resize its children to their preferred size, but will
 * not set their location. This program does not do anything useful.
 *
 * The program sets the locations of a canvas that shows a checkerboard and a label.
 * The pane will set the size of the label. Since a canvas is not resizable, it is simply
 * shown at its natural size. Two buttons are also shown. They are set to be "unmanaged" which
 * stops any container from setting their size and positions. This makes it possible to set the
 * size of the buttons as well as their locations.
 */

public class TheCheckerBoard extends Application {

    /**
     * Canvas on which a checker board is drawn, defined by a static nested subclass.
     */
    CheckerBoard board;
     ...
     * size of the Pane is set explicitly to 500-by-420; the stage will take
     * its size from the preferred size of the Pane. Four child nodes are added to the
     * Pane. The location of each child is set, and sizes are set for two buttons.
     * The buttons are set to be unmanaged to stop the Pane from setting their sizes.
     * ( Without the preferred size for the Pane, it would be just large enough to show
     * the checkerboard and the label, but not the buttons, which it ignores. )
     */
    public void start( Stage stage ) {
        ...
        /**
         * Set the sizes of the buttons. For this to have any effect, make the buttons
    ...
    /**
     * This canvas displays a 320-by-320 checkerboard patters with a 2-pixel
    ...
    private static class CheckerBoard extends Canvas {
        ...
        public void draw() {
            ...
            // Draw the checkerboard pattern in a gray and lightGray color.
