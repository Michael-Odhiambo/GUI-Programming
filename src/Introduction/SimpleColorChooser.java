...
/**
 * This program shows six sliders that the user can manipulate to set the
 * red, green, blue, hue, brightness, and saturation components of a color.
 * A color patch shows the selected color, and there are six labels that
 * show the numerical values of all the components.
 */
...
public class SimpleColorChooser extends Application {
    ...
    public void start( Stage stage ) {
        ...
        redSlider = new Slider( 0, 1, 0 );
        greenSlider = new Slider( 0, 1, 0 );
        blueSlider = new Slider( 0, 1, 0 );

        /* Set up listeners to respond when the slider value is changed.*/
    ...
    private Label makeText( String message ) {
        // Make a label to show a given message shown in bold, with some padding between the text and the border of the label.
    ...
    private void newColor( Slider whichSlider ) {
        ...
        if ( !whichSlider.isValueChanging() ) {
            // Don not respond to change if it was set programmatically;
            // Only respond if it was set by user draggind the slider.
            return;
        }
        Color color;
